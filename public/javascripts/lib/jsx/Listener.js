...
namespace('jsx').Class( 'Listener').def({
    Listener: function( id, element, type, scope, func, config ) {
        ...
        this.container = true; /* wird momentan nur bei mouseout verwendet */
        this.relations = [];
    ...
    _posInElement: function() {
        ...
        return { x: Math.round(posInDocument.x - elementPos.left), y: Math.round(posInDocument.y - elementPos.top)};
    },

    _posInContent: function() {
        var posInDocument = this._posInDocument();
        var contentPos = this.element.positionContent();
        return { x: posInDocument.x - contentPos[0], y: posInDocument.y - contentPos[1]};
    },
    ...
    toString: function() { return '[' + this.id + ' Listener]'; }
})
    .Static({
        _idCounter: 0,
        _ID_PREFIX: 'lsr:',
